...
namespace NexusMods.Monitor.Scraper.Host.BackgroundServices
{
    public sealed class NexusModsIssueMonitor : CriticalBackgroundService
    {
        ...
        public NexusModsIssueMonitor(ILogger<NexusModsIssueMonitor> logger, IServiceScopeFactory scopeFactory, IApplicationEnder applicationEnder) : base(applicationEnder)
        {
        ...
        {
            using var loggingScope = _logger.BeginScope("Service: {Service}", nameof(NexusModsIssueMonitor));
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                await policy.ExecuteAsync(async ct => await _timeLimiter.Enqueue(async () =>
                {
                    ...
                    var processor = scope.ServiceProvider.GetRequiredService<NexusModsIssuesProcessor>();
