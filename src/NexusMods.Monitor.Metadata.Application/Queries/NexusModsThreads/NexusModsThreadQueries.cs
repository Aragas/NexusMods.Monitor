...
namespace NexusMods.Monitor.Metadata.Application.Queries.NexusModsThreads
{
    public sealed class NexusModsThreadQueries : INexusModsThreadQueries
    {
        ...
        public NexusModsThreadQueries(IHttpClientFactory httpClientFactory, INexusModsGameQueries nexusModsGameQueries, IMemoryCache cache)
        {
        ...
        public async Task<NexusModsThreadViewModel> GetAsync(uint gameId, uint modId, CancellationToken ct = default)
        {
            var key = $"thread_id({gameId}, {modId})";
            if (!_cache.TryGetValue(key, out NexusModsThreadViewModel cacheEntry))
            {
