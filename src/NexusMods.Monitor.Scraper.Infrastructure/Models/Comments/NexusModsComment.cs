...
namespace NexusMods.Monitor.Scraper.Infrastructure.Models.Comments
{
    public class NexusModsComment
    {
        ...
        public List<NexusModsCommentReply> Children { get; } = new List<NexusModsCommentReply>();

        public NexusModsComment(IElement element) // comment-container
        {
            ...
            foreach (var subComment in kids?.Children ?? Enumerable.Empty<IElement>())
                Children.Add(new NexusModsCommentReply(subComment, Id));
        }
