...
namespace NexusMods.Monitor.Bot.Slack.Application.CommandHandlers
{
    public sealed class Unsubscribe2CommandHandler : IRequestHandler<Unsubscribe2Command, bool>
    {
        ...
        public Unsubscribe2CommandHandler(ILogger<Unsubscribe2CommandHandler> logger, IHttpClientFactory httpClientFactory, IOptions<SubscriptionsOptions> options, DefaultJsonSerializer jsonSerializer)
        {
        ...
        public async Task<bool> Handle(Unsubscribe2Command message, CancellationToken ct)
        {
            var response = await _httpClientFactory.CreateClient().PutAsync(
                $"{_options.APIEndpointV1}/unsubscribe2",
